...
const Catalog: React.FC<
  CatalogProps & { handleBoardChange: (boardName: string) => void }
> = ({ boardsData, handleBoardChange }) => {
  ...
  return (
    <ul
      className={
        className +
        " " +
        "w-full tablet:flex tablet:my-0 my-2 px-4 tablet:px-12 flex items-center justify-center flex-wrap gap-3"
      }
    >
      {boardsData.map((boardName) => (
        <li key={boardName}>
          <button
            onClick={() => handleClick(boardName)}
            className={`inline-flex items-center small_phone:gap-2 gap-0 justify-center text-base px-4 tablet:px-11 py-2 tablet:py-4 my-2 tablet:my-4 rounded-full font-semibold shadow-sm shadow-[#0000001F] ${
              activeItem === boardName.toLowerCase()
          ...
          >
            ...
            <span className="ml-1 leading-5 small_phone:text-base text-xs">
              {boardName}
            </span>
            <svg
